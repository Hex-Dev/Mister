...
#######################################################################################################
############ MRA Alternative Sega System 16  ##########################################################

echo " "
printf "${Red}MRA Alternative $System : ${NC}\n"
echo " "


IFS=$'\n'       # make newlines the only separator
# for file in $(find /media/fat/_Arcade/_alternatives -name '*.mra')
for file in $(find /media/fat/_Arcade/_alternatives -maxdepth 2 -type f -iname *.mra)
do
    if grep -q ">jts16" $file 
	then
        echo -n "Copy : " 
